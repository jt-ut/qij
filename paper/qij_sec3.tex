...
We evaluate QIJ on three datasets: two arise from distributional families in which every quantity is analytically available <<is this true for MVT distribution? My code is doing simulations here>>, and one is a real-world example from observational astronomy showcasing vector-valued interval estimates. Throughout, $S=1000$ Monte Carlo draws of size $N = 1000$ are taken from each data process; QIJ and a standard nonparametric bootstrap ($B = 2000$) run on identical draws, paired by seed \todo{confirm $M$ grids} <<M grids vary for each DGP>>. The astronomical case uses an empirical population protocol: a large catalogue is treated as the population, draws are sampled from it with replacement, and the target $\theta_{\mathrm{true}}$ are taken to be the parameter estimates computed from the full catalogue. Below, we describe the datasets individually. 

\paragraph{Pareto.} A heavy-tailed distribution, with closed-form influence functions for both estimators (the shape parameter's maximum-likelihood estimator, and a tail probability), so every stage of the pipeline can be checked analytically.

\paragraph{Multivariate $t$.} A ten-dimensional and elliptically symmetric distribution, with the degrees of freedom $\nu$ estimated by profile maximum likelihood; this tests that the construction survives higher data dimension. 

\paragraph{Galaxy fundamental plane.} The galaxy Fundamental Plane dataset (FP) comprises 76,997 early-type galaxies drawn from the Sloan Digital Sky Survey Data Release 7 spectroscopic catalog. Galaxies were selected in the redshift range 0.05 ≤ z ≤ 0.10 with reliable velocity dispersion measurements (σ > 70 km/s, σ_err < 30 km/s) and concentration indices r₉₀/r₅₀ > 2.5 to isolate early-type morphologies. For each galaxy, the three Fundamental Plane observables are the aperture-corrected stellar velocity dispersion σ (following the Jorgensen et al. 1995 correction), the de Vaucouleurs effective radius R_e converted to physical kpc using a standard ΛCDM cosmology (H₀ = 70 km/s/Mpc, Ω_m = 0.3), and a luminosity-based surface brightness proxy I_e = L_r / (2πR_e²) derived from the extinction-corrected r-band apparent magnitude. The Fundamental Plane is fitted in log-space as log R_e = a · log σ + b · log I_e + c via weighted least squares. $(a,b,c)$ is a vector of estimands. 
